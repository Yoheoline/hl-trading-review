...
→ **65%以上** かつ **confluence≧2** → 実行。それ以外 → 見送り。
...
```
→ Account 3にポジションありなら **即終了（NO_REPLY）**

### 【4. シグナル取得】
```powershell
pwsh -Command "Set-Location 'C:/Users/yohei/Projects/hyperliquid-bot/src/executor/node-executor'; node check-signals.js 5m"
```

### 【5. 判断】
- 確信度計算（上記の式）
- **65%未満** or **confluence<2** → **即終了（NO_REPLY）**
...
### 【7. エントリー】
```powershell
pwsh -Command "Set-Location 'C:/Users/yohei/Projects/hyperliquid-bot/src/executor/node-executor'; node executor.js market_open BTC risk_pct 2.0 <slPct> <isLong> --account 3"
pwsh -Command "Set-Location 'C:/Users/yohei/Projects/hyperliquid-bot/src/executor/node-executor'; node add-tpsl-v2.js --account 3"
```
...
エントリー実行時のみ。フォーマット:
```
⚡ [Account 3] BTC [LONG/SHORT] @ $XX,XXX
...
- 「サクッと入ってサクッと出る」 — 執着しない
- 速さが命。迷う時間は損失に直結する
- 小さな利益を笑うな。積み重ねが大きな差になる
- 連敗3回で一旦冷却。冷静さを取り戻してから再開
