...
(async () => {
  ...
  for (let i = closes.length - 14; i < closes.length; i++) {
    const diff = closes[i] - closes[i - 1];
    if (diff > 0) g14 += diff; else l14 -= diff;
  }
  const rsi14 = l14 === 0 ? 100 : 100 - (100 / (1 + (g14/14) / (l14/14)));
  console.log('RSI(14):', rsi14.toFixed(1));

  // Momentum(2) - change over last 2 candles
  ...
  if (rsi14 < 25) console.log('=> RSI(14) OVERSOLD - LONG signal');
  if (rsi14 > 80) console.log('=> RSI(14) OVERBOUGHT - SHORT signal');
